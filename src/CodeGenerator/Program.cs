...
internal static partial class Program
{
    static void Main(string[] args)
    {
        ...
        {
            using CSharpCodeWriter writer = new(Path.Combine(outputPath, ed.FriendlyNames[0] + ".gen.cs"));
        ...
        foreach (TypeDefinition td in defs.Types)
        {
            ...
            using CSharpCodeWriter writer = new(Path.Combine(outputPath, td.Name + ".gen.cs"));
            writer.Using("System");
            ...
            foreach (FunctionDefinition fd in defs.Functions)
            {
                foreach (OverloadDefinition overload in fd.Overloads)
                {
                    ...
                    KeyValuePair<string, string>[] orderedDefaults = [.. overload.DefaultValues.OrderByDescending(
                            kvp => GetIndex(overload.Parameters, kvp.Key))];
        ...
        using (CSharpCodeWriter writer = new(Path.Combine(outputPath, $"{classPrefix}Native.gen.cs")))
        {
            writer.Using("System");
            writer.Using("System.Numerics");
        ...
        using (CSharpCodeWriter writer = new(Path.Combine(outputPath, $"{classPrefix}.gen.cs")))
        {
            writer.Using("System");
            writer.Using("System.Numerics");
            writer.Using("System.Runtime.InteropServices");
    ...
    {
        return FilenameFieldRegex().IsMatch(name)
            || NameFieldRegex().IsMatch(name);
    }
        ...
        string classPrefix)
    {
        ...
        List<(string MarshalledType, string CorrectedIdentifier)> invocationArgs = [];
        ...
        for (int i = 0; i < overload.Parameters.Length; i++)
        {
            ...
            else if (tr.Type == "void*" || tr.Type == "ImWchar*")
            {
                ...
                && !TypeInfo.WellKnownTypes.ContainsKey(tr.Type[..^1]))
            {
            ...
            else if ((tr.Type.EndsWith("*") || tr.Type.Contains("[") || tr.Type.EndsWith("&")) && tr.Type != "void*" && tr.Type != "ImGuiContext*" && tr.Type != "ImPlotContext*" && tr.Type != "EditorContext*")
            {
                ...
                if (tr.Type.Contains("["))
                {
                    ...
                    nonPtrType = GetTypeString(wellKnown[..^1], false);
                }
                else
                {
                    nonPtrType = GetTypeString(tr.Type[..^1], false);
                }
                string nativeArgName = "native_" + tr.Name;
    ...
    private static string GetTypeString(string typeName, bool isFunctionPointer)
    {
        ...
        {
            if (TypeInfo.WellKnownTypes.TryGetValue(typeName[..^pointerLevel], out typeStr))
            {
    ...
    private static partial Regex FilenameFieldRegex();
    [GeneratedRegex(".*Name")]
    private static partial Regex NameFieldRegex();
}

class MarshalledParameter(string marshalledType, bool isPinned, string varName, bool hasDefaultValue)
{
    public string MarshalledType { get; } = marshalledType;
